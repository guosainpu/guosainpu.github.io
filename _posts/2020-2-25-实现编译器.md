...
程序结构类的非终结符用于分析class的结构，变量声明的结构，函数声明、函数实现的等结构：
...
有很多算法可以用来构建语法分析树，递归下降算法是比较明显和简单的一种。语法分析程序从左到右的读取词法分析器的token输出，遇到非终结符则对齐进行相应的展开，如果该非终结符仍由其他非终结符构成，则深度优先的递归这些非终结符，直至非终结符全部由终结符构成为止。
<center><img src="https://tva1.sinaimg.cn/large/0082zybply1gcb14smxp3j31900u0gus.jpg" width="800"></center>
