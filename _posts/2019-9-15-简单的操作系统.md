...
显示器的内存映射是从16384开始的连续的 8K\*2byte 地址空间（因此该显示器的分辨率是：512\*256= 8K\*2byte）；键盘更简单，它只映射24576这一个地址单元的地址空间（因为足以容纳所有英文字符和符号）
...
可以看到，键盘输入的设计实现还是基于一系列级联的分层抽象。高级程序readLine()依赖readChar()，readChar()依赖于keyPress()，keyPress()依赖于硬件。可见分层抽象的设计模式贯穿了整个计算机的宏观架构和微观实现，可以说是计算机科学思想的精华所在。
