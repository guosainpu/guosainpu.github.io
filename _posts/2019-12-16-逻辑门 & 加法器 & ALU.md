上篇说道：通过使用Nand门，我们可以实现任何逻辑门，进而实现可以一个CPU。后面我们就会搭建一个麻雀虽小但五脏俱全的计算机平台：hack。**本篇我们开始第一步，实现搭建hack所需的一组芯片：组合逻辑芯片**
### 组合逻辑芯片
一个最基本的CPU主要由两类重要的的芯片组成：

* 组合逻辑芯片（Combinational Chips）：与或非门、加法器、ALU芯片等。这些芯片主要负责逻辑计算
...
### 基本逻辑门
基本逻辑门是几种简单布尔函数的实现，我们需要的基本逻辑门如下，有1位和多位（16位）之分
![](https://tva1.sinaimg.cn/large/006tNbRwly1g9yzjn8lngj30qe0mgwew.jpg)

<center><img src="https://tva1.sinaimg.cn/large/006tNbRwly1g9yzjn8lngj30qe0mgwew.jpg" width="50%"></center>

首先我们先用Nand实现一个Not。下面是Nand的真值表，只有当两个输入都为1时输出为0，其余输出为1反，然后输出。
![](https://tva1.sinaimg.cn/large/006tNbRwly1ga16e7ws5tj30b007s3yd.jpg)

Not很简单，对其输入取反，然后输出。下面给出Not的实现电路图和HDL语言。

![](https://tva1.sinaimg.cn/large/006tNbRwly1ga16vpl6lqj30s60860so.jpg)
...
![](https://tva1.sinaimg.cn/large/006tNbRwly1ga173gnszgj311g082dfu.jpg)
...
多位基本逻辑门是一位基本逻辑门的简单组合，以16位And为例，用16个1位And组合成16位And
```
/**
 * 16-bit bitwise And:
...
### 加法器
有了上面的基本逻辑门有什么用呢？我们利用这些基本逻辑门，就可以实现CPU中一个非常重要的基本芯片：加法器。实现加法器的顺序是半加器->全加器->16位加法器



第一步：实现一个半加器。半加器负责计算两个一位二进制相加，输出sum和carry（进位）。
![](https://tva1.sinaimg.cn/large/006tNbRwly1ga1xb8b2xej30sc09m74d.jpg)

虽然半加器看上去由布尔运算升级到的小学数学，但仍然可以用真值表表示，所以也就可以用基本逻辑门实现，我们用一个Xor和And就可以实现半加器。
电路图：
![](https://tva1.sinaimg.cn/large/006tNbRwly1ga1xkdvxztj30rw0eq74e.jpg)
...
半加器只能计算两个一位二进制相加，接着我们在输入端加入一个进位管脚（代表低位做加法时进过来的进位），这样就实现了全加器。
![](https://tva1.sinaimg.cn/large/006tNbRwly1ga1xom01j9j30u80de0sv.jpg)
电路图：
![](https://tva1.sinaimg.cn/large/006tNbRwly1ga1y4fai2lj318k0eq0sy.jpg)
...
![](https://tva1.sinaimg.cn/large/006tNbRwly1ga1xrljwy9j30te06sdfv.jpg)
HDL:
```
/**
...
有了加法器，我们再实现一个计数器，它对输入数据进行加1，然后输出。计数器是后面CPU中程序计数器的重要部分
```
CHIP Inc16 {
    IN in[16];
    OUT out[16];
...
### 多通道逻辑门
我们看到基本逻辑门负责计算布尔运算，只能根据输入计算出输出，缺少控制能力。下面我们介绍的多通道逻辑门则在逻辑控制方面扮演了重要角色，因为多通道逻辑门具有**选择功能**，多通道逻辑门在CPU和RAM中起到了重要作用。多通道逻辑门分为以下两种：

* **Multiplexor:** 有多路输入，一路输出，选择输入中的一路作为输出
* **DMultiplexor:** 有一路输入，多路输出，在输出路中选择一路降输入值输出

![](https://tva1.sinaimg.cn/large/006tNbRwly1ga1y8t42i3j30p60dq0sv.jpg)
![](https://tva1.sinaimg.cn/large/006tNbRwly1ga1y9h0fixj30me08mjrd.jpg)


这里我们实现一个1位Mux：
图：
![](https://tva1.sinaimg.cn/large/006tNbRwly1ga1zhhi4a2j30rm0kuq36.jpg)
Hdl:
```
/** 
 * Multiplexor:
...
当然除了两路选择器，还有4路、8路、16路等多路选择器。
![](https://tva1.sinaimg.cn/large/006tNbRwly1ga1yd61q3mj30ze09at8v.jpg)
HDL:4路16位选择器
```
/**
...
ALU示意图：
![](https://tva1.sinaimg.cn/large/006tNbRwly1ga1yf3rz82j30pi0i0dg1.jpg)

ALU真值表：
![](https://tva1.sinaimg.cn/large/006tNbRwly1ga1yf94f4ej314x0u0jsn.jpg)
...
### 补码
最后我们再来谈一下补码，上面我们提到的加法器都没有考虑正负符号问题，如果引入符号，那么电路会变得更加复杂。通过使用补码可以很好的规避这个问题，因为补码有一个很重要的特性：任何两个用补码表示的有符号数的加法和正数的加法完全相同，例如用正数加法操作（-2）+（-3），所得的结果正好是（-5）的补码；补码的减法可以用加法来代替实现，例如x-y == x+(-y)。
